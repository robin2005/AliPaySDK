	...
	if (self = [super init]) {
        ...
        else
        {
            do {
                ...
                if([jsonMemo objectForKey:@"ResultStatus"])
                {
                    self.statusCode = [[jsonMemo objectForKey:@"ResultStatus"] intValue];
                }
                else if([jsonMemo objectForKey:@"resultStatus"])
                {
                    self.statusCode = [[jsonMemo objectForKey:@"resultStatus"] intValue];
                }
                else if([jsonMemo objectForKey:@"resultstatus"])
...
- (id)initWithDictionray:(NSDictionary *)dic
{
    ...
    if (self = [super init]) {
        ...
        self.statusCode = 0;
        self.statusMessage = @"未知错误。";
        NSDictionary * jsonMemo = dic;
        do {
            
            if (![jsonMemo isKindOfClass:[NSDictionary class]]) {
                break;
            }
